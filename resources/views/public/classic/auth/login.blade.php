  ...
  <div class="container ep-auth-page">
    <div class="ep-auth-card contact-form">
      ...
      <form method="post" action="{{ route('login.submit') }}" class="auth-form-fields">
        ...
        <button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;margin-top:4px">Sign
          in</button>
      </form>
      <p style="margin:16px 0 0;font-size:14px;color:var(--muted);text-align:center">
        Don’t have an account? <a href="{{ route('register', array_filter(['redirect' => $authRedirectPath ?? null])) }}"
          style="color:var(--primary-2);font-weight:600">Create one</a>
      </p>
      <p style="margin:12px 0 0;font-size:13px;color:var(--muted);text-align:center">
        Organizer or staff? <a href="{{ route('admin.login') }}"
          style="color:var(--muted);font-weight:600;text-decoration:underline;text-underline-offset:3px">Staff sign in</a>
      </p>
    </div>
  </div>
@endsection
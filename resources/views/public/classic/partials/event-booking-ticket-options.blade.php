  ...
  @foreach($event->additionalServices as $svc)
    ...
    <div class="option" data-kind="addon" data-name="{{ $svc->name }}" data-price="{{ number_format((float) $svc->price, 2, '.', '') }}">
      ...
      <div class="qty">
        ...
        <button type="button" class="plus" onclick="changeQty(this,1)" @if($oldA >= 50) disabled @endif>+</button>
        <input type="hidden" class="qty-hidden" name="{{ $addonName }}" value="{{ $oldA }}" data-max="50" autocomplete="off" />
      </div>
    </div>
  @endforeach
</div>

      ...
      <ul class="account-booking-list">
        ...
        <li class="account-booking-row">
          <div>
            ...
            @endif
            <div class="meta">Booking #{{ $ticketBooking->id }} ·
              {{ $ticketBooking->created_at?->format('M j, Y g:i A') }}</div>
          </div>
            <div class="account-booking-actions">
              <span class="badge-status">{{ str_replace('_', ' ', (string) $ticketBooking->status) }}</span>
              <a href="{{ route('account.bookings.ticket-pdf', $ticketBooking) }}" class="account-link">Download ticket
                PDF</a>
            </div>
          </li>
        @endforeach
